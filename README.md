...
Untrusted input is an alert; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — an alert
- **Closed type:** `type Decision = Page(Team) | Ack` where `Team` is closed
- **Sink / capability:** `grant page` → `privileged { page(d) }`
- **Consequence axes:** Trust

## Run the demo

```sh
examples/apps/alert-router/demo.sh
```
...
- `alert-router_safe.eide` — the correct design.
- `alert-router_unsafe.eide` — the tempting-but-wrong version (the negative example a model must learn to reject).
